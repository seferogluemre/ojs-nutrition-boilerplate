...
async function syncCategories(): Promise<void> {
  ...
  try {
    ...
    for (const mainCategory of categories) {
      ...
      if (mainCategory.id) {
        categoryMapping.set(mainCategory.id, dbMainCategory.id);
      }
      ...
      if (mainCategory.children) {
        for (const childCategory of mainCategory.children) {
          ...
          // Store mapping if external category has ID
          if (childCategory.id) {
            categoryMapping.set(childCategory.id, dbChildCategory.id);
          }
            ...
            for (const subChildCategory of childCategory.sub_children) {
              const subChildCategoryUuid = generateUUID();
               ...
               if (dbSubChildCategory) {
                 // Update existing sub child category
              ...
              console.log(`    └─ Sub kategori: ${subChildCategory.name}`);
            }
          }
        }
      }
  ...
  try {
    ...
    for (const product of products) {
      try {
        ...
        const categoryId = categoryMapping.get(productDetail.sub_category_id);
        if (!categoryId) {
